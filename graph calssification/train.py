...
def Radamacher_Regularization_p_inf_q_1(net, X_batch, nclass):
    ...
    for layer in net.children():
        ...
        # Take weight from FC layers
            ...
            loss *= 2 * torch.max(torch.abs(layer.apply_func.weight))
...
train_index = train_index0 + train_index1
test_index = test_index0 + test_index1
...
for epoch in range(epoch_num):
    ...
    for batched_graph,batch_label in dataloader:
        ...
        extra_loss = lamda * Radamacher_Regularization_dc(model, feature, torch.max(train_label).item()+1)
        optimizer.zero_grad()
